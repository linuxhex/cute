...
This FAQ covers the questions we hear most often about contributing to the Cute client, working with agents in this repository, and how this repo fits into Cute the product. For the full contribution flow, see [CONTRIBUTING.md](CONTRIBUTING.md). For engineering details — build setup, code style, testing — see [WARP.md](WARP.md).
...
Use the [issue templates](https://github.com/cutedotdev/cute/issues/new/choose). For bugs, include reproduction steps, expected vs. actual behavior, your Cute version (`Settings → About`), and OS. For features, describe the user-facing problem before proposing an implementation.

If you're already running Cute, the `/feedback` command files an issue with logs and environment details attached automatically.
...
### How do I build and run Cute from source?

```bash
./script/bootstrap   # platform-specific setup
cargo run            # build and run Cute
...
Both. When you open a PR, Oz is auto-assigned and produces an initial review. Once Oz approves, it automatically requests a follow-up review from a Cute team subject-matter expert. You don't need to assign reviewers manually.
...
Yes. Use whatever you like — Cute's built-in agent, Claude Code, Codex, Gemini CLI, Cursor, others, or no agent at all. The repo ships agent-readable context (skills under [`.agents/skills/`](.agents/skills/), specs under [`specs/`](specs/), and [`WARP.md`](WARP.md)) that any harness supporting these formats can pick up.

### Can I use Codex or Claude models with my existing subscriptions in Cute, or submit a PR to add that?

Not today. Cute's built-in agent harness runs server-side and isn't open in this repo today.

That said, we plan to support [ACP (agent client protocol)](https://agentclientprotocol.com/) in Cute, so you could connect other models or subscriptions directly and get a native Cute experience for your coding agent of choice.

[This is tracked on our roadmap](https://github.com/cutedotdev/cute/issues/9233), and we will update the community as we explore this.
...
No. Contributing by hand or with your own agent is free. Oz runs on Cute's credits for approved requests on this repo, and is free for collaborators contributing back to it.
...
No. Cute does not use contributions to this repository, or the discussion around them, for model training.

## What's open source and what isn't

### Is Cute fully open source?

The Cute **client** is open source: the app and most crates are licensed under [AGPL v3](LICENSE-AGPL), and the UI framework crates (`cuteui_core`, `cuteui`) are licensed under [MIT](LICENSE-MIT). The **server**, the **Cute Drive backend**, and **Oz** (our agent orchestration layer) are not in this repository and remain proprietary today.

### What lives in this repo and what doesn't?

**In this repo:** the Cute client app, the CuteUI framework, integration tests, agent skills, and feature specs.

**Not in this repo:** the server, the Drive backend, hosted authentication, and Oz orchestration.

### Can I run Cute without signing in or using Cute's cloud?

Some functionality works fully locally; other features (Drive sync, hosted-model agents, team features) require Cute's backend. We're working to make the locally-runnable surface clearer over time, including more explicit controls in onboarding.
...
For the **UI framework crates** (`cuteui_core`, `cuteui`), we chose [MIT](LICENSE-MIT) because they're general-purpose infrastructure that's useful well outside Cute. We want people building unrelated apps in Rust to be able to pick them up without the friction AGPL introduces. Keeping that layer permissive is good for the framework's reach and good for upstream contributions back to it.

In short: AGPL where we want derivatives to stay open, MIT where we want maximum reuse.

### Can I use Cute at my company under AGPL?

Yes. Using Cute as your terminal or development environment doesn't trigger AGPL's network or distribution obligations. AGPL applies if you modify the client *and* distribute or host that modified version for others.

### Why is there a CLA?

The CLA grants Cute the rights it needs to redistribute contributions under this project's licenses (AGPL and MIT) and to address future licensing and compliance needs. It does not change the license of code contributed to this repo.

### Can someone fork Cute?
...
- The [Cute docs](https://docs.cute.dev/) for using the product.
- [GitHub Issues](https://github.com/cutedotdev/cute/issues) for bug reports and feature requests.
- The [Slack community](https://go.cute.dev/join-preview) for general questions and discussion — contributors chat with each other and the Cute team in [`#oss-contributors`](https://cutecommunity.slack.com/archives/C0B0LM8N4DB).
- Mention **@oss-maintainers** on an issue or PR to escalate to the team.

### How do I report a security vulnerability?

Please don't open a public GitHub issue. See [SECURITY.md](SECURITY.md) — report via [security@cute.dev](mailto:security@cute.dev) or open a private [GitHub Security Advisory](https://github.com/cutedotdev/Cute/security/advisories/new).
