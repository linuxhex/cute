#include "CuteDockTilePlugin.h"

@implementation CuteDockTilePlugIn {
...
- (instancetype)init {
    ...
    if (self) {
        @try {
            ...
            [self logMessage:@"CuteDockTilePlugin initialized"];
...
- (void)setDockTile:(NSDockTile *)dockTile {
    @try {
        ...
        if (dockTile) {
            ...
            NSBundle *pluginBundle = [NSBundle bundleForClass:[CuteDockTilePlugIn class]];    
            NSString *path = [[pluginBundle bundlePath] stringByAppendingPathComponent:@"Contents/Info.plist"];
            NSDictionary *dict = [NSDictionary dictionaryWithContentsOfFile:path];    
            NSString *bundleId = dict[@"MainAppBundleIdentifier"];
...
- (void)dealloc {
    @try {
        [self logMessage:@"CuteDockTilePlugin deallocating"];
