...
use cuteui::{AppContext, ModelHandle, SingletonEntity, ViewHandle, WindowId};
...
#[cfg(test)]
mod testing {
    use cuteui::platform::WindowStyle;
    use cuteui::{App, Element, SingletonEntity};
    ...
    impl cuteui::Entity for TerminalRootView {
        type Event = ();
    }

    impl cuteui::View for TerminalRootView {
        fn ui_name() -> &'static str {
            "TerminalRootView"
        }

        fn render(&self, _app: &cuteui::AppContext) -> Box<dyn cuteui::Element> {
            cuteui::elements::ChildView::new(&self.terminal_view).finish()
        }
    }

    impl cuteui::TypedActionView for TerminalRootView {
        type Action = ();
    }
