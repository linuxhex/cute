use cuteui::{EntityId, ModelContext, ModelHandle, SingletonEntity};
...
fn create_handler(agent: &CLIAgent) -> Option<Box<dyn CLIAgentSessionHandler>> {
    match agent {
        ...
        | CLIAgent::Pi
        | CLIAgent::Qoder
        | CLIAgent::Trae => Some(Box::new(DefaultSessionListener)),
...
impl cuteui::Entity for CLIAgentSessionListener {
    type Event = ();
}
