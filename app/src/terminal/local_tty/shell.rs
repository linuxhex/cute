...
use cute_util::path::{canonicalize_git_bash_path, is_msys2_path, warp_shell_path};
...
pub fn extra_path_entries() -> impl Iterator<Item = PathBuf> {
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            ...
            if let Some(resources_path) = cute_core::paths::bundled_resources_dir() {
...
impl WslShellStarter {
    ...
    pub(super) fn home_directory(&self) -> Option<PathBuf> {
        ...
        cute_util::path::convert_wsl_to_windows_host_path(
