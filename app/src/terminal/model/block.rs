...
use cute_terminal::model::grid::Dimensions as _;
...
use cuteui::record_trace_event;
use cuteui::units::{IntoLines, Lines};
...
impl Block {
    ...
    pub fn top_level_command(&self, sessions: &Sessions) -> Option<String> {
        ...
        let command = cute_completer::parsers::simple::top_level_command(
        ...
        session
            ...
            .and_then(|s| cute_completer::parsers::simple::top_level_command(s, escape_char))
    ...
    fn wakeup_after_delay(&self) {
        ...
        self.background_executor
            .spawn(async move {
                cuteui::r#async::Timer::after(std::time::Duration::from_millis(delay_ms)).await;
