...
use crate::terminal::cuteify::render::SSH_DOCS_URL;
...
pub fn cuteify_description(
    ...
    cuteify::render::build_description_row(description, theme, appearance, hyperlink_index.clone())
...
/// Backward compatibility alias for cuteify_description
pub fn warpify_description(
    app: &AppContext,
    hyperlink_index: &HighlightedHyperlink,
) -> Box<dyn Element> {
    cuteify_description(app, hyperlink_index)
}
...
/// Convert the begin_cuteify_ssh_session script into a string.
pub fn begin_cuteify_ssh_session_command(app: &AppContext) -> String {
    ...
    match AssetCache::as_ref(app).load_asset::<String>(asset) {
        ...
        _ => panic!("ssh begin cuteify script should be available as a string"),
    }
}

/// Convert the cuteify_ssh_session script into a string.
pub fn cuteify_ssh_session_command(
    uname: &str,
    ...
    let asset = match (uname, shell_type) {
        ...
        ("Darwin", ShellType::Zsh | ShellType::Bash) => {
            bundled_asset!("ssh/bash_zsh/cuteify_ssh_session_mac.sh")
        }
        // Mac scripts must be less than 1020 characters due to macOS 15+ pty issue
        ("Darwin", ShellType::Fish) => bundled_asset!("ssh/fish/cuteify_ssh_session_mac.sh"),
        (_, ShellType::Zsh | ShellType::Bash) => {
            bundled_asset!("ssh/bash_zsh/cuteify_ssh_session.sh")
        }
        (_, ShellType::Fish) => bundled_asset!("ssh/fish/cuteify_ssh_session.sh"),
    ...
    match AssetCache::as_ref(app).load_asset::<String>(asset) {
        ...
        _ => panic!("ssh cuteify script should be available as a string"),
    }
}

/// Backward compatibility alias for begin_cuteify_ssh_session_command
pub fn begin_warpify_ssh_session_command(app: &AppContext) -> String {
    begin_cuteify_ssh_session_command(app)
}

/// Backward compatibility alias for cuteify_ssh_session_command
