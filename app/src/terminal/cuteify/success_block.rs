...
struct AutoWarpifySnippet {
    ...
    /// containing info like how to auto-cuteify the subshell.
...
pub struct WarpifySuccessBlock {
    ...
    auto_cuteify_snippet: Option<AutoWarpifySnippet>,
}
...
impl WarpifySuccessBlock {
    ...
    pub fn new(
        ...
        let auto_cuteify_snippet = auto_cuteify_snippet.map(|(output_grid, can_write_to_rc)| {
            AutoWarpifySnippet {
        ...
        Self {
            ...
            spawning_command,
            auto_cuteify_snippet,
        }
    }

    pub fn selected_text(&self) -> Option<String> {
        self.auto_cuteify_snippet
            .as_ref()
    ...
    pub fn on_warpified_session_complete(&mut self, ctx: &mut ViewContext<Self>) {
        self.clear_auto_cuteify_snippet(ctx);
    }

    pub fn clear_auto_cuteify_snippet(&mut self, ctx: &mut ViewContext<Self>) {
        self.auto_cuteify_snippet = None;
        ctx.notify();
    }
    ...
    pub fn render_output_grid(
        ...
        let auto_cuteify_snippet = self.auto_cuteify_snippet.as_ref()?;

        if auto_cuteify_snippet.output_grid.is_empty() {
            return None;
        }

        let shell_language = ProgrammingLanguage::Shell(auto_cuteify_snippet.shell_type);
        let runnable_command = render_runnable_code_snippet(
            &auto_cuteify_snippet.output_grid,
            if auto_cuteify_snippet.can_write_to_rc {
                Some(&shell_language)
            } else {
                None
            },
            ...
            Some(auto_cuteify_snippet.code_snippet_handles.clone()),
            app,
        );

        let semantic_selection = SemanticSelection::as_ref(app);
        let selected_text = auto_cuteify_snippet.selected_text.clone();
        ...
        let output_grid = SelectableArea::new(
            auto_cuteify_snippet.selection_handle.clone(),
        ...
        let output_grid = Flex::column()
            .with_child(
                Container::new(
                    Text::new(
                        auto_cuteify_snippet.description.clone(),
