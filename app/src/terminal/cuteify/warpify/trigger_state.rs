...
use crate::terminal::ssh::cuteify::SshWarpifyBlock;
...
struct WarpifyTriggerState {
    ...
    /// Lets us abort an attempt to auto cuteify if the subshell command
    /// hasn't completed.
    auto_cuteify_abort_handle: Option<SpawnedFutureHandle>,
...
impl WarpifyState {
    ...
    pub fn add_auto_cuteify_abort_handle(&mut self, spawned_future_handle: SpawnedFutureHandle) {
        let pending_state = self.pending_state.get_or_insert_with(Default::default);
        pending_state.auto_cuteify_abort_handle = Some(spawned_future_handle);
    }

    pub fn abort_auto_cuteify(&mut self) {
        if let Some(abort_handle) = self
            .pending_state
            .as_mut()
            .and_then(|state| state.auto_cuteify_abort_handle.take())
        {
            abort_handle.abort();
        };
    }

    pub fn add_ssh_cuteify_timeout_handle(&mut self, spawned_future_handle: SpawnedFutureHandle) {
        let pending_state = self.pending_state.get_or_insert_with(Default::default);
        pending_state.ssh_cuteify_timeout_handle = Some(spawned_future_handle);
    }

    pub fn abort_ssh_cuteify_timeout(&mut self) {
        self.replace_timeout_id();
        if let Some(handle) = self
            .pending_state
            .as_mut()
            .and_then(|state| state.ssh_cuteify_timeout_handle.take())
        {
            handle.abort();
        };
    }
    ...
    pub fn get_pending_ssh_host(&self) -> Option<String> {
        self.pending_state
            .as_ref()
            .and_then(|state: &WarpifyTriggerState| state.pending_cuteify_ssh_host.clone())
    }
    ...
    pub fn take_pending_ssh_host(&mut self) -> Option<String> {
        self.pending_state
            .as_mut()
            .and_then(|state: &mut WarpifyTriggerState| state.pending_cuteify_ssh_host.take())
    }

    pub fn clear_pending_ssh_host(&mut self) {
        if let Some(ref mut pending_state) = self.pending_state.as_mut() {
            pending_state.pending_cuteify_ssh_host = None;
        }
    }
    ...
    pub fn set_pending_ssh_host(&mut self, command: String, ssh_host: Option<String>) {
        ...
        pending_state.pending_cuteify_ssh_host = ssh_host;
    }
