...
impl TryFrom<cute_graphql::ai::AIConversationArtifact> for Artifact {
    type Error = ();

    fn try_from(value: cute_graphql::ai::AIConversationArtifact) -> Result<Self, Self::Error> {
        match value {
            cute_graphql::ai::AIConversationArtifact::PlanArtifact(plan) => Ok(Artifact::Plan {
            ...
            cute_graphql::ai::AIConversationArtifact::PullRequestArtifact(pr) => {
                let (repo, number) = parse_github_pr_url(&pr.url).unzip();
            ...
            cute_graphql::ai::AIConversationArtifact::ScreenshotArtifact(screenshot) => {
            ...
            cute_graphql::ai::AIConversationArtifact::FileArtifact(file) => Ok(Artifact::File {
            ...
            cute_graphql::ai::AIConversationArtifact::Unknown => Err(()),
        }
    }
}
...
pub fn open_screenshot_lightbox<V: cuteui::View>(
    artifact_uids: &[String],
    ctx: &mut cuteui::ViewContext<V>,
) {
...
pub fn download_file_artifact<V: cuteui::View>(
    artifact_uid: &str,
    ctx: &mut cuteui::ViewContext<V>,
) {
    let ai_client = ServerApiProvider::handle(ctx).as_ref(ctx).get_ai_client();
...
fn open_file_download_result<V: cuteui::View>(
    artifact_uid: &str,
...
#[cfg(feature = "local_fs")]
fn open_file_download_picker<V: cuteui::View>(
    artifact: ArtifactDownloadResponse,
    ctx: &mut cuteui::ViewContext<V>,
) {
    ...
    ctx.open_save_file_picker(
        move |path_opt: Option<String>, _me: &mut V, ctx: &mut cuteui::ViewContext<V>| {
...
fn show_file_download_toast<V: cuteui::View>(
    artifact_uid: &str,
