...
use cute_graphql::mutations::expire_api_key::ExpireApiKeyResult;
use cute_graphql::mutations::generate_api_key::GenerateApiKeyResult;
use cute_graphql::queries::api_keys::ApiKeyProperties;
use cute_graphql::scalars::Time;
            ...
            async move {
                ...
                let result = match result {
                    ...
                    GenerateApiKeyResult::UserFacingError(e) => {
                        return Err(anyhow!(
                            cute_graphql::client::get_user_facing_error_message(e)
                        ));
                    }
...
impl cuteui::Entity for ApiKeyCommandRunner {
    type Event = ();
}
...
fn print_created_api_key(
    ...
    output_format: OutputFormat,
    json_output: cute_cli::json_filter::JsonOutput,
) -> Result<()> {
