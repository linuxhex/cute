...
impl AgentManagementRunner {
    fn spawn_command(
        &self,
        future: impl cuteui::r#async::Spawnable<Output = anyhow::Result<()>>,
...
fn print_skills_hint() {
    let binary_name = cute_cli::binary_name().unwrap_or_else(|| "warp".to_string());
...
impl cuteui::Entity for AgentManagementRunner {
    type Event = ();
}
