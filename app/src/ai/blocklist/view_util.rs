        ...
        if state.is_hovered() {
            ...
            let tool_tip = appearance
                ...
                .with_style(UiComponentStyles {
                    font_size: Some(12.),
                    background: Some(cuteui::elements::Fill::Solid(tooltip_background)),
...
fn render_input_icon(icon: Icon, color: Fill, app: &AppContext) -> Box<dyn Element> {
    ...
    ConstrainedBox::new(
        Container::new(icon.to_cuteui_icon(color).finish())
    ...
    F: FnMut(&mut EventContext) + 'static,
{
    ...
    let mut label_children = vec![ConstrainedBox::new(icon.to_cuteui_icon(color).finish())
