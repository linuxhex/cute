...
fn resolve_interactive_defaults(
    ...
    if state.orch.model_id.is_empty() {
        let harness =
            cute_cli::agent::Harness::parse_orchestration_harness(&state.orch.harness_type);
        if matches!(harness, Some(cute_cli::agent::Harness::Oz) | None) {
...
impl RunAgentsCardView {
    ...
    pub fn update_request(&mut self, request: &RunAgentsRequest, ctx: &mut ViewContext<Self>) {
        ...
        if new_state.orch.model_id.is_empty() {
            let harness =
                cute_cli::agent::Harness::parse_orchestration_harness(&new_state.orch.harness_type);
            if matches!(harness, Some(cute_cli::agent::Harness::Oz) | None) {
    ...
    fn maybe_auto_open_create_modal(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let Some(harness) =
            cute_cli::agent::Harness::parse_orchestration_harness(&self.state.orch.harness_type)
        else {
            return;
        };
        ...
        if self.handles.pickers.host_picker.is_none() {
            ...
            handle.update(ctx, |picker, picker_ctx| {
                picker.set_menu_position(
                    cuteui::elements::PositionedElementAnchor::TopLeft,
                    cuteui::elements::ChildAnchor::BottomLeft,
                    picker_ctx,
                );
            });
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            RunAgentsCardViewAction::CreateNewAuthSecretRequested => {
                ...
                if let Some(harness) = cute_cli::agent::Harness::parse_orchestration_harness(
                    &self.state.orch.harness_type,
                ) {
                    ctx.dispatch_typed_action(
                        &crate::workspace::WorkspaceAction::OpenCreateAuthSecretModal { harness },
...
fn render_editor(
    ...
    use cuteui::elements::ConstrainedBox;
    let appearance = Appearance::as_ref(app);
    let theme = appearance.theme();
    let mut column = Flex::column().with_cross_axis_alignment(CrossAxisAlignment::Stretch);
