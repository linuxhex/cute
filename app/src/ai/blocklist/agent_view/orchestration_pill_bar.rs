...
use cuteui::platform::{Cursor, LineStyle};
...
fn render_hover_card(
    ...
    // the shared `cute_util::path::user_friendly_path` helper so the cwd
    ...
    let cwd_line: Option<Box<dyn Element>> = conversation
        ...
        .map(|cwd| {
            Text::new(
                cute_util::path::user_friendly_path(&cwd, home_dir_str).into_owned(),
                appearance.ui_font_family(),
    ...
    let card = Container::new(column.finish())
        ...
        .with_background(bg)
        .with_border(cuteui::elements::Border::all(1.).with_border_fill(outline))
...
fn render_status_badge(
    ...
    let icon_el = ConstrainedBox::new(icon.to_cuteui_icon(color.into()).finish())
        .with_width(12.)
        .with_height(12.)
        .finish();
    let label = Text::new(
...
fn render_pin_glyph_centered(is_pinned: bool, icon_color: ColorU) -> Box<dyn Element> {
    ...
    let glyph: Box<dyn Element> =
        ConstrainedBox::new(icon_variant.to_cuteui_icon(icon_color.into()).finish())
...
fn render_pill(
    ...
    self_terminal_view_id: cuteui::EntityId,
    app: &AppContext,
) -> Box<dyn Element> {
    let appearance = Appearance::as_ref(app);
    let theme = appearance.theme();
    ...
    // `crates/cuteui_core/src/elements/hoverable.rs`). We can therefore move
...
fn render_overflow_button(
    ...
    let button = Hoverable::new(mouse_state, move |hover_state| {
        ...
        let icon = ConstrainedBox::new(Icon::DotsVertical.to_cuteui_icon(text_color).finish())
            .with_width(PILL_ICON_SIZE)
            .with_height(PILL_ICON_SIZE)
            .finish();
