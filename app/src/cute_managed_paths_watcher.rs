...
pub(crate) fn ensure_cute_watch_roots_exist() {
    ...
    let config_local_dir = cute_core::paths::config_local_dir();
...
pub(crate) fn cute_home_config_dir() -> Option<PathBuf> {
    cute_core::paths::cute_home_config_dir()
}

pub(crate) fn cute_home_skills_dir() -> Option<PathBuf> {
    cute_core::paths::cute_home_skills_dir()
}

#[cfg_attr(target_family = "wasm", allow(dead_code))]
pub(crate) fn cute_home_mcp_config_file_path() -> Option<PathBuf> {
    cute_core::paths::cute_home_mcp_config_file_path()
}
...
impl CuteManagedPathsWatcher {
    ...
    fn new_internal(ctx: &mut ModelContext<Self>, should_register_watcher: bool) -> Self {
        ...
        if should_register_watcher {
            ...
            if let (Some(cute_home_config_dir), Some(_cute_home_mcp_config_path)) =
    ...
    fn register_path(
        ...
        let registration = watcher.update(ctx, |watcher, _ctx| {
            watcher.register_path(&registration_path, recursive_mode)
        });
...
mod tests {
    ...
    use super::{
        filter_repository_update_by_prefix, cute_home_mcp_config_file_path, cute_home_skills_dir,
        cute_managed_mcp_config_path, cute_managed_skill_dirs,
    };
    ...
    fn filter_repository_update_by_prefix_keeps_only_matching_paths() {
        let skills_dir = PathBuf::from("/tmp/.cute-local/skills");
        let other_dir = PathBuf::from("/tmp/.cute-local/worktrees/repo");
    ...
    fn filter_repository_update_by_prefix_converts_cross_boundary_moves() {
        let skills_dir = PathBuf::from("/tmp/.cute-local/skills");
        let skill_file = skills_dir.join("deploy").join("SKILL.md");
        let ignored_file = PathBuf::from("/tmp/.cute-local/worktrees/repo/SKILL.md");
