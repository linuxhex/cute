...
use crate::pane_group::{
    self, AIFactPane, AnyPaneContent, ChildAgentOrigin, CodeDiffPane, CodePane,
...
impl Workspace {
    ...
    fn handle_import_modal_event(&mut self, event: &ImportModalEvent, ctx: &mut ViewContext<Self>) {
        match event {
            ...
            ImportModalEvent::Cancel | ImportModalEvent::Import => {
                self.current_workspace_state.is_import_modal_open = false;
                ctx.notify();
            }
        }
    ...
    fn build_workflow_modal(
        ...
        let workflow_modal =
            ctx.add_view(|ctx| WorkflowModal::new(ai_client.clone(), ctx));
        ...
        let close_button = Hoverable::new(
            ...
            |hover_state| {
                let icon = ConstrainedBox::new(
                    icons::Icon::X
                        .to_cuteui_icon(Fill::Solid(PhenomenonStyle::modal_close_button_text()))
                        .finish(),
                )
                .with_width(16.)
                .with_height(16.)
    ...
    pub fn new(
        ...
        _server_time: Option<Arc<ServerTime>>,
        ...
        // Simplified: local version has no referral theme status
        ...
        ctx.subscribe_to_model(&OneTimeModalModel::handle(ctx), |me, model, event, ctx| {
            ...
            if *is_open {
                ...
                if model_ref.target_window_id() == Some(ctx.window_id()) {
                    if model_ref.is_hoa_onboarding_open() {
        ...
        match workspace_setting {
            ...
            NewWorkspaceSource::Restored {
                ...
                // 本地模式跳过云 pane 后，若 tab 无有效 pane 则补一个终端 tab
                {
                    ...
                    if any_empty_tab {
                        log::warn!(
                            "Restored workspace has empty tabs after skipping cloud panes; resetting to a fresh session tab"
                        );
                        self.tabs.clear();
    ...
    fn restore_left_panel_for_tab(
        ...
        self.left_panel_view.update(ctx, |lp, ctx| {
            ...
            let active_view = match left_panel_snapshot.left_panel_displayed_tab {
                ...
                _ => ToolPanelView::ProjectExplorer,
            };
    ...
    fn stop_sharing_session(&mut self, _terminal_view_id: EntityId, _ctx: &mut ViewContext<Self>) {}

    fn copy_shared_session_link_from_tab(&mut self, _tab_index: usize, _ctx: &mut ViewContext<Self>) {}

    fn copy_shared_session_link(&mut self, _session_id: &SharedSessionId, _ctx: &mut ViewContext<Self>) {
        // Shared session link copying has been removed
    }
    ...
    fn stop_sharing_all_panes_in_tab(
        ...
        if let Some(pane_group) = pane_group.upgrade(ctx) {
            let _shared_views = pane_group.as_ref(ctx).shared_session_view_ids(ctx);
        }
    }
    ...
    fn reset_focused_index_in_cute_drive(
        &mut self,
        _should_scroll: bool,
        _ctx: &mut ViewContext<Self>,
    ) {
        // ctx.focus(&self.left_panel_view);
        // self.update_cute_drive_view(ctx, |drive_panel, ctx| {
        //     drive_panel.reset_focused_index_in_cute_drive(should_scroll, ctx);
        // });
    }

    pub fn has_cute_drive_initialized_sections(
        &self,
        app: &AppContext,
    ) -> bool {
        let _ = app;
    ...
    /// Check if Cute Drive view is focused within.
    /// Routes to the appropriate Cute Drive panel.
    fn is_cute_drive_view_focused(&self, ctx: &mut ViewContext<Self>) -> bool {
        let app = ctx;
    ...
    fn focus_left_panel(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        else if self.is_cute_drive_view_focused(ctx) {
    ...
    fn focus_right_panel(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        // Starts from a left panel: theme chooser (Warp Drive disabled)
        else if self.theme_chooser_view.is_self_or_child_focused(ctx)
        {
            self.focus_active_tab(ctx);
        }
    ...
    fn view_logs(&mut self, ctx: &mut ViewContext<Self>) {
        ctx.spawn(
            async { tokio::task::spawn_blocking(cute_logging::create_log_bundle_zip).await },
    ...
    fn export_all_cute_drive_objects(&mut self, ctx: &mut ViewContext<Self>) {
        let window_id = ctx.window_id();
        let cloud_model = CloudModel::as_ref(ctx);
    ...
    fn open_launch_config_from_menu(
        ...
        match new_session_menu_item {
            NewSessionMenuItem::OpenLaunchConfig(launch_config) => ctx.dispatch_global_action(
                ...
                OpenLaunchConfigArg {
                    launch_config,
                    ui_location: LaunchConfigUiLocation::TAB_MENU,
            ...
            Arc::new(move |_, _, appearance, _| {
                ...
                let search_icon = ConstrainedBox::new(
                    icons::Icon::SearchSmall
                        .to_cuteui_icon(theme.sub_text_color(theme.surface_2()))
                        .finish(),
                )
                .with_width(16.)
                .with_height(16.)
    ...
    fn configure_worktree_new_session_sidecar(
        ...
        self.new_session_sidecar_menu.update(ctx, |menu, view_ctx| {
            ...
            menu.set_pinned_footer_builder(move |app| {
                ...
                .with_cursor(Cursor::PointingHand)
                .on_click(|ctx: &mut cuteui::elements::EventContext, _, _| {
    ...
    fn open_repo_picker_for_tab_config_modal(
        ...
        ctx.open_file_picker(
            ...
            cuteui::platform::FilePickerConfiguration::new().folders_only(),
        );
    }
    ...
    fn maybe_generate_worktree_name(
        ...
        Some(cute_util::worktree_names::generate_worktree_branch_name(
            &branch_refs,
        ))
    }
    ...
    fn handle_new_worktree_submit(
        ...
        let filename_hint = if let Some(name) = worktree_branch_name {
            ...
            let branch_refs: HashSet<&str> = branches.iter().map(|s| s.as_str()).collect();
            cute_util::worktree_names::generate_worktree_branch_name(&branch_refs)
        };
    ...
    fn open_worktree_in_repo(&mut self, repo_path: String, ctx: &mut ViewContext<Self>) {
        ...
        let branch_refs: HashSet<&str> = branches.iter().map(|s| s.as_str()).collect();
        let branch_name = cute_util::worktree_names::generate_worktree_branch_name(&branch_refs);
    ...
    fn handle_workflow_modal_event(
        ...
        match event {
            ...
            WorkflowModalEvent::CreateWorkflow => {
                // No action needed in stub version
            }
    ...
    fn handle_auth_manager_event(
        ...
        match event {
            AuthManagerEvent::AttemptedLoginGatedFeature { auth_view_variant: _ } => {
                // Cute: 本地版始终已登录，忽略登录门控事件
            }
            AuthManagerEvent::LoginOverrideDetected(_interrupted_auth_payload) => {
                // Cute: 本地版始终已登录，忽略登录覆盖事件
            }
    ...
    pub fn open_autoupdate_failure_link(&mut self, ctx: &mut ViewContext<Self>) {
        // Autoupdate removed - redirect to general docs
        ctx.open_url("https://docs.warp.dev/");
    }
    ...
    pub fn sync_window_button_visibility(&self, ctx: &mut ViewContext<Self>) {
        use cuteui::platform::mac::WindowExt;
    ...
    fn open_ctrl_tab_palette(
        ...
        match query_filter {
            QueryFilter::Tabs => {
                // tab_navigation_data disabled - skip this filter
            }
                        ...
                        ActiveSession::as_ref(ctx)
                            ...
                            .and_then(|wd| match wd {
                                LocalOrRemotePath::Remote(remote) => {
                                    let std_path =
                                        cute_util::standardized_path::StandardizedPath::try_new(
                                            path,
                                        )
                                        .ok()?;
                                    Some(LocalOrRemotePath::Remote(
                                        cute_util::remote_path::RemotePath::new(
    ...
    fn view_in_cute_drive(&mut self, item_id: WarpDriveItemId, ctx: &mut ViewContext<Self>) {
        self.open_left_panel(ctx);
        // self.left_panel_view.update(ctx, |left_panel, ctx| {
        ...
        if let WarpDriveItemId::Object(object_id) = &item_id {
            CloudModel::handle(ctx).update(ctx, |model, ctx| {
                model.force_expand_object_and_ancestors_cloud_id(object_id.clone(), ctx);
            });
        }
    ...
    pub fn view_in_and_focus_warp_drive(
        ...
        self.view_in_cute_drive(item_id.clone(), ctx);
    ...
    fn open_object_sharing_settings(
        ...
        self.view_in_cute_drive(WarpDriveItemId::Object(object_id.clone()), ctx);
        self.update_warp_drive_view(ctx, |warp_drive, ctx| {
            warp_drive.reset_and_open_to_main_index(ctx);
    ...
    fn move_to_drive_space(
        ...
        self.update_warp_drive_view(ctx, |warp_drive, ctx| {
            warp_drive.move_object_to_team_owner(cloud_object_type_and_id, ctx);
        });
    }

    fn set_focused_index(&mut self, _index: Option<usize>, ctx: &mut ViewContext<Self>) {
        self.update_warp_drive_view(ctx, |warp_drive, ctx| {
            warp_drive.set_focused_index(ctx);
        });
        ctx.notify();
    }
    ...
    /// group. This feature has been removed - the function is kept as a stub
    /// for UI compatibility.
    pub(crate) fn open_network_log_pane(&mut self, _ctx: &mut ViewContext<Self>) {
        // Network log feature has been removed. This stub exists for UI compatibility.
    }
    ...
    // Removed: all local-to-cloud handoff functions - not needed in local version
    ...
    pub(crate) fn handle_file_tree_event(
        ...
        match event {
            ...
            pane_group::Event::MoveToSpace {
                ...
                self.move_to_drive_space(cloud_object_type_and_id.clone(), *space, ctx);
            }
            ...
            pane_group::Event::OpenWarpDriveObjectInPane(uid) => {
                self.open_cute_drive_object_in_new_pane(uid, ctx);
            }
                ...
                if self.active_tab_pane_group().as_ref(ctx).left_panel_open && is_target_active {
                    ...
                    self.left_panel_view.update(ctx, |left_panel, ctx| {
                        let action = match target_view {
                            ...
                            LeftPanelTargetView::WarpDrive => LeftPanelAction::ProjectExplorer,
                        };
        ...
        if self.is_input_box_visible(ctx) {
            ...
            self.toast_stack.update(ctx, |toast_stack, ctx| {
                ...
                if let Some(id) = object_id {
                    toast = toast.with_object_id(id.uid().to_string());
                }
                toast_stack.add_ephemeral_toast(toast, ctx);
            });
            return None;
    ...
    fn set_selected_object(&mut self, id: Option<WarpDriveItemId>, ctx: &mut ViewContext<Self>) {
        ...
        self.update_warp_drive_view(ctx, |drive_panel, ctx| {
            drive_panel.set_selected_object(id.clone(), ctx);
        });
        // If WD open, show the highlighted object (force expand necessary ancestors)
        if self.current_workspace_state.is_warp_drive_open {
            if let Some(id) = id {
                self.view_in_cute_drive(id, ctx);
                ctx.notify();
            }
        }
    }
    ...
    /// Simplified: always returns false for local version (no free tier modal).
    ...
    fn open_workflow_with_temporary(&mut self, _workflow: Workflow, _ctx: &mut ViewContext<Self>) {
        // 注释掉云端个人存储功能 - 本地版本不支持
    ...
    fn open_workflow_with_command(&mut self, _command: String, _ctx: &mut ViewContext<Self>) {
        // 注释掉云端个人存储功能 - 本地版本不支持
        // let Some(owner) = UserWorkspaces::as_ref(ctx).personal_drive(ctx) else {
    ...
    fn render_right_panel_button(
        ...
        let icon = ConstrainedBox::new(icons::Icon::Diff.to_cuteui_icon(font_color).finish())
            .with_width(16.)
            .with_height(16.)
            .finish();
        ...
        Hoverable::new(
            ...
            |mouse_state| {
                let row = Flex::row()
                    ...
                    .with_child(
                        ConstrainedBox::new(
                            icons::Icon::Search.to_cuteui_icon(text_color).finish(),
                        )
                        .with_width(16.)
                        .with_height(16.)
                        .finish(),
    ...
    fn render_tab_overflow_menu(
        ...
        // Autoupdate removed - no tab overflow menu
        None
    }
    ...
    fn banner_fields(&self, app: &AppContext) -> Option<WorkspaceBannerFields> {
        ...
        // than that they continue to see any of the crash recovery banners.
    ...
    fn render_workspace_banner(
        ...
        let icon =
            ConstrainedBox::new(Icon::AlertCircle.to_cuteui_icon(text_color.into()).finish())
                .with_width(16.)
                .with_height(16.)
                .finish();
                        ...
                        move |state| {
                            let mut container = Container::new(
                                ConstrainedBox::new(
                                    ...
                                    Icon::X.to_cuteui_icon(text_color.into()).finish(),
                                )
                                .with_width(16.)
                                .with_height(16.)
                                .finish(),
    ...
    fn render_config_panel(
        ...
        match item {
            ...
            HeaderToolbarItemKind::ToolsPanel => {
                if !pane_group.left_panel_open || cuteui::platform::is_mobile_device() {
                    return None;
                }
    ...
    fn add_toggle_setting_context_flags(&self, app: &AppContext, context: &mut Context) {
        ...
        let cuteify_settings = CuteifySettings::as_ref(app);
        ...
        if *cuteify_settings.enable_ssh_warpification.value() {
            context.set.insert(flags::SSH_WARPIFICATION_CONTEXT_FLAG);
        }

        if *cuteify_settings.use_ssh_tmux_wrapper.value() {
    ...
    fn compute_left_panel_views(ctx: &AppContext) -> Vec<ToolPanelView> {
        ...
        // if CuteDriveSettings::is_warp_drive_enabled(ctx) {
...
impl TypedActionView for Workspace {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            // Simplified: removed ShowUpgrade and ShowReferralSettingsPage handlers
            ...
            CopyAccessTokenToClipboard => {
                ...
                let access_token =
                    cuteui::r#async::block_on(self.server_api.get_or_refresh_access_token());
            ...
            StopSharingSessionFromTabMenu { terminal_view_id } => {
                self.stop_sharing_session(*terminal_view_id, ctx)
            }
            ...
            OpenSharedSessionQrCode { session_id } => {
                // Shared session QR code functionality has been removed
                let _ = session_id;
            }
            ...
            OpenOrAttachAmbientAgentConversation {
                    ...
                    self.find_pane_with_ambient_agent_conversation(*task_id, ctx)
                {
                    ...
                    self.add_tab_for_joining_shared_session(session_id.clone(), ctx);
                }
            }
...
impl View for Workspace {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        {
            let pane_group = self.active_tab_pane_group().as_ref(app);
            if cuteui::platform::wasm::is_mobile_device() && pane_group.left_panel_open {
                let scrim = Rect::new()
