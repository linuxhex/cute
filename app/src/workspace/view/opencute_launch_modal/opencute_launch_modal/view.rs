...
const HERO_IMAGE_PATH: &str = "async/png/onboarding/opencute_launch_banner.png";
...
const FEATURE_ITEMS: &[FeatureItem] = &[
    FeatureItem {
        icon: Icon::HeartHand,
        title: "Contribute",
        description: "Cute's client code is now open source. Get started by using the /feedback skill to open an issue, and follow the contribution guidelines here.",
    ...
    FeatureItem {
        ...
        description: "The Cute repo is managed by an agent-first workflow powered by Oz, our cloud agent orchestration platform.",
        inline_link: Some(InlineLink {
            text: "Oz",
            url: OZ_URL,
        }),
...
pub fn init(app: &mut AppContext) {
    ...
    app.register_fixed_bindings([FixedBinding::new(
        "escape",
        OpenCuteLaunchModalAction::Close,
        id!(OpenCuteLaunchModal::ui_name()),
    )]);
}

#[derive(Clone, Debug)]
pub enum OpenCuteLaunchModalAction {
    Close,
    VisitRepo,
}

#[derive(Clone, Debug)]
pub enum OpenCuteLaunchModalEvent {
    Close,
}
...
impl OpenCuteLaunchModal {
    ...
    fn render_title(appearance: &Appearance) -> Box<dyn Element> {
        Text::new("Cute is now open-source", appearance.ui_font_family(), 20.)
    ...
    fn render_description(appearance: &Appearance) -> Box<dyn Element> {
        Text::new(
            "You, our community, can participate in building Cute using an agent-first workflow.",
...
impl Entity for OpenCuteLaunchModal {
    type Event = OpenCuteLaunchModalEvent;
}

impl View for OpenCuteLaunchModal {
...
impl TypedActionView for OpenCuteLaunchModal {
    type Action = OpenCuteLaunchModalAction;

    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            OpenCuteLaunchModalAction::Close => {
                ctx.emit(OpenCuteLaunchModalEvent::Close);
            }
            OpenCuteLaunchModalAction::VisitRepo => {
