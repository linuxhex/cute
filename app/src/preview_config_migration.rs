...
//! directory (`~/.cute-preview`) on macOS.
//!
//! Historically, Stable and Preview shared `~/.cute` on macOS. To give
//! Preview its own directory without breaking existing users, this migration
//! symlinks each top-level entry from `~/.cute` into `~/.cute-preview` on
...
use cute_core::paths::{data_dir, WARP_CONFIG_DIR};
...
/// Migrates Preview's config directory from the shared `.cute` location to
/// `.cute-preview` by creating symlinks from each top-level entry in `.cute`
...
pub(crate) fn migrate_config_dir_via_symlinks(old_dir: &Path, new_dir: &Path) {
    ...
    // migration itself or by ensure_cute_watch_roots_exist on a subsequent
