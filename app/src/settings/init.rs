...
use cuteui_extras::user_preferences;
...
pub fn register_all_settings(ctx: &mut AppContext) {
    ...
    // CloudAgentSettings::register(ctx); // Cute: 已注释，清理云端 Agent 设置注册
...
pub fn init(
    ...
    #[cfg(feature = "local_fs")]
    {
        let prefs = <settings::PublicPreferences as cuteui::SingletonEntity>::as_ref(ctx);
...
fn handle_warp_config_change(
    _: cuteui::ModelHandle<crate::user_config::WarpConfig>,
...
fn init_platform_native_preferences() -> user_preferences::Model {
    cfg_if::cfg_if! {
        if #[cfg(test)] {
            ...
            let app_id = cute_core::channel::ChannelState::app_id();
            ...
            Box::new(user_preferences::user_defaults::UserDefaultsPreferencesStorage::new(
                cute_core::channel::ChannelState::data_domain_if_not_default()
            ))
...
pub fn init_public_user_preferences() -> (user_preferences::Model, Option<user_preferences::Error>)
{
    cfg_if::cfg_if! {
        if #[cfg(test)] {
            ...
            if cute_core::features::FeatureFlag::SettingsFile.is_enabled() {
