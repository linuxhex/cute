...
use cute_editor::content::text::BufferBlockItem;
use cuteui::elements::{
...
use cuteui::{AppContext, Element, SingletonEntity, ViewContext, ViewHandle};
    ...
    fn create_menu(
            ...
            menu.add_item(
                MenuItemFields::new(block_type.label())
                    ...
                    .with_on_select_action(EditorViewAction::InsertBlock(
                        cute_editor::content::text::BlockType::Text(block_type.into()),
                    ))
                    .into_item(),
            );
        }
        ...
        menu.add_item(
            MenuItemFields::new("Divider")
                ...
                .with_on_select_action(EditorViewAction::InsertBlock(
                    cute_editor::content::text::BlockType::Item(BufferBlockItem::HorizontalRule),
                ))
...
impl RichTextEditorView {
    ...
    fn insert_embedded_workflow(&mut self, id: &SyncId, ctx: &mut ViewContext<Self>) {
        self.insert_block(
            cute_editor::content::text::BlockType::Item(BufferBlockItem::Embedded {
    ...
    fn insert_embedded_notebook(&mut self, id: &SyncId, ctx: &mut ViewContext<Self>) {
        let (title, link) = CloudModel::handle(ctx).read(ctx, |model, _| {
            ...
            let link = model
                .get_by_uid(&CloudObjectTypeAndId::Notebook(*id).uid().uid())
