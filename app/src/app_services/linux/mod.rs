...
pub enum StartupArgsForwardingError {
    /// There's no instance of Cute already running.
...
impl From<zbus::fdo::Error> for StartupArgsForwardingError {
    fn from(value: zbus::fdo::Error) -> Self {
        ...
        // registering a service, so this really means that Cute is not already
        // running.
...
// A D-Bus client for connecting to an already-running instance of Cute and
// invoking org.freedesktop.Application IPC methods.
#[proxy(
    interface = "org.freedesktop.Application",
    default_service = "dev.cute.CuteLocal",
    default_path = "/dev/cute/CuteLocal",
    gen_blocking = false
)]
