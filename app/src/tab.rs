...
use cute_core::ui::theme::AnsiColors;
use cuteui::elements::{
...
impl TabStyles {
    ...
    fn default(appearance: &Appearance, tab_color: Option<AnsiColorIdentifier>) -> TabStyles {
        ...
        let sharing_color = ColorU::default();
    ...
    fn render_indicator(&self) -> Option<Box<dyn Element>> {
        let icon = match &self.indicator {
            ...
            Indicator::Synced => Some(
                Icon::LinkHorizontal
                    .to_cuteui_icon(self.styles.synced_input_indicator_color.into())
                    .finish(),
            ),
            Indicator::Error => Some(
                Icon::AlertTriangle
                    .to_cuteui_icon(self.styles.error_color.into())
                    .finish(),
            ),
            Indicator::Shared => Some(
                Icon::Sharing
                    .to_cuteui_icon(self.styles.sharing_color.into())
                    .finish(),
            ),
            ...
            Indicator::Shell(shell_indicator_type) => Some(
                shell_indicator_type
                    .to_icon()
                    .to_cuteui_icon(internal_colors::neutral_5(self.appearance.theme()).into())
                    .finish(),
            ),
            Indicator::Agent {
                conversation_status,
                ...
                if let Some(status) = conversation_status {
                    ...
                    Some(Icon::Oz.to_cuteui_icon(icon_color).finish())
                }
            }
            Indicator::AmbientAgent => {
                ...
                Some(Icon::OzCloud.to_cuteui_icon(icon_color.into()).finish())
            }
                    ...
                    let tooltip_content: Box<dyn Element> = if has_extra_info {
                        ...
                        if let Some(directory) = &tooltip_directory_clone {
                            let folder_icon = Icon::Folder
                                .to_cuteui_icon(ThemeFill::Solid(font_color))
                                .finish();
