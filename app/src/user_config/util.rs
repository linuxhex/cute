...
use crate::themes::theme::{ThemeKind, CuteTheme, CuteThemeConfig};
...
pub(super) fn parse_single_theme_dir_entry(item: &DirEntry) -> Option<(ThemeKind, CuteTheme)> {
    parse_single_item_file(item, |file_name, mut theme: CuteTheme| {
        ...
        let theme_kind = if let Some(name) = theme.name() {
            CuteThemeConfig::file_to_theme(name, item.path().into())
        } else {
