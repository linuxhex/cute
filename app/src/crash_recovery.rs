...
use cute_cli::RecoveryMechanism;
...
/// Keep in sync with [`cute_cli::AppArgs`].
...
pub fn is_crash_recovery_process(args: &cute_cli::AppArgs) -> bool {
...
impl CrashRecoveryProcess {
    ...
    fn kill(&mut self) {
        ...
        *IS_CRASH_RECOVERY_PROCESS_RUNNING.write() = false;
        self.is_alive = false;
        cute_logging::on_crash_recovery_process_killed();
    }
...
impl CrashRecovery {
    ...
    #[cfg(test)]
    pub fn register_for_test(app: &mut cuteui::App) {
...
#[cfg(windows)]
fn wait_for_parent_crash(args: &cute_cli::AppArgs) {
...
fn handle_parent_crash(
    ...
    cute_logging::on_parent_process_crash();
