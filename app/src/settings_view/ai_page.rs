...
impl AISettingsPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let custom_inference_controls_enabled =
            is_any_ai_enabled && UserWorkspaces::as_ref(ctx).is_custom_inference_enabled(); // 删除：云端功能已禁用 - 删除参数
...
impl SettingsWidget for UsageWidget {
    ...
    fn render(
        ...
        // Simplified: no delinquency check for local version
        let workspace_is_delinquent_due_to_payment_issue = false;
        ...
        // Simplified: no upgrade CTA for local version
        let upgrade_cta_text_fragments = vec![
...
impl ApiKeysWidget {
    fn new(ctx: &mut ViewContext<<Self as SettingsWidget>::View>) -> Self {
        ...
        let is_byo_enabled = workspace_handle.as_ref(ctx).is_byo_api_key_enabled(); // 删除：云端功能已禁用 - 删除参数
            ...
            ($editor:ident, $key:ident, $set_func:ident, $placeholder:literal) => {
                ...
                ctx.subscribe_to_model(&workspace_handle, move |_, workspace, event, ctx| {
                    if let UserWorkspacesEvent::TeamsChanged = event {
                        ...
                        let is_byo_enabled = workspace.as_ref(ctx).is_byo_api_key_enabled(); // 删除：云端功能已禁用 - 删除参数
...
impl SettingsWidget for ApiKeysWidget {
    ...
    fn render(
        ...
        let ai_settings = AISettings::as_ref(app);
        let is_any_ai_enabled = ai_settings.is_any_ai_enabled(app);
        let is_byo_enabled = UserWorkspaces::as_ref(app).is_byo_api_key_enabled(); // 删除：云端功能已禁用 - 删除参数
...
impl AwsBedrockWidget {
    ...
    fn render_aws_bedrock_section(
        ...
        let toggle_description = if is_admin_enforced {
            "Cute loads and sends local AWS CLI credentials for Bedrock-supported models. This setting is managed by your organization.".to_string()
        } else {
            "Cute loads and sends local AWS CLI credentials for Bedrock-supported models."
                .to_string()
        };
