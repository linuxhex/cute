...
use cute_core::semantic_selection::{
...
use cuteui::keymap::{ContextPredicate, FixedBinding, Keystroke};
use cuteui::platform::{Cursor, GraphicsBackend};
use cuteui::rendering::GPUPowerPreference;
...
use {
    ...
    cuteui::platform::linux::windowing_system_is_customizable,
};
    ...
    fn handle_action(&mut self, action: &FeaturesPageAction, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            ToggleForceX11 => {
                ...
                self.page.scroll_by(cuteui::units::Pixels::new(40.));
                ctx.notify();
            }
            ToggleQuitOnLastWindowClosed => {
                GeneralSettings::handle(ctx).update(ctx, |settings, ctx| {
...
impl SettingsPageMeta for FeaturesPageView {
    ...
    fn on_page_selected(&mut self, _: bool, ctx: &mut ViewContext<Self>) {
        // On MacOS, we rely on [`cuteui::platform::AppCallbacks::on_screen_changed`] to update and
...
impl SettingsWidget for DefaultTerminalWidget {
    ...
    fn search_terms(&self) -> &str {
        "cute default terminal application"
    }
    ...
    fn render(
        ...
        if default_terminal.is_warp_default() {
            ui_builder
                .wrappable_text("Cute is the default terminal", true)
                .with_style(UiComponentStyles {
