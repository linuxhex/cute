...
use cuteui::ui_components::segmented_control::{
...
pub enum CreateApiKeyModalEvent {
    Close,
    Created {
        api_key: cute_graphql::queries::api_keys::ApiKeyProperties,
    },
    Error {
        message: String,
    },
...
impl CreateApiKeyModal {
    ...
    fn create(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let expires_at = match self.expiration.days() {
            Some(days) => {
                ...
                Some(cute_graphql::scalars::Time::from(t))
            }
            None => None,
        };
    ...
    fn render_success_content(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let copy_icon = if self.raw_key_copied {
            cute_core::ui::icons::Icon::Check.to_cuteui_icon(appearance.theme().background())
        } else {
            cute_core::ui::icons::Icon::Copy
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            CreateApiKeyModalAction::CopyRawKey => {
                ...
                ctx.clipboard()
                    .write(cuteui::clipboard::ClipboardContent::plain_text(content));
