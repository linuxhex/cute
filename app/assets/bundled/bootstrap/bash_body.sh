...
if [ -z "$WARP_BOOTSTRAPPED" ]; then
    ...
    cute_send_json_message () {
        ...
        encoded_message=$(cute_hex_encode_string "$1")
    ...
    cute_send_hook_via_kv_pairs_end () {
    ...
    #
    # Usage:
    #   cute_send_generator_output_osc $my_message
    #
    ...
    cute_send_generator_output_osc () {
        ...
        cute_send_generator_output_osc_pre_hex_encoded "$hex_encoded_message"
    }

    # Note: If we're on windows, we send a reset grid to erase any cursor mutations caused by
    # the in-band command.
    cute_send_generator_output_osc_pre_hex_encoded () {
    ...
    _cute_execute_command() {
      ...
      cute_send_generator_output_osc_pre_hex_encoded "$generator_output"
    }
    ...
    cute_preexec () {
        ...
        if [ "$WARP_IN_MSYS2" = true ]; then
          cute_send_hook_via_kv_pairs_start "Preexec"
          cute_send_hook_kv_pair "command" "$BASH_COMMAND"
          cute_send_hook_via_kv_pairs_end
        else
          local truncated_command=$(cute_escape_json "$BASH_COMMAND")
          cute_send_json_message "{\"hook\": \"Preexec\", \"value\": {\"command\": \"$truncated_command\"}}"
        fi
        cute_maybe_send_reset_grid_osc
        ...
        if [[ "$BASH_COMMAND" != cute_run_generator_command* ]] && [[ -f $_WARP_GENERATOR_PIDS_STARTED_TMP_FILE ]] && [[ -f $_WARP_GENERATOR_PIDS_COMPLETED_TMP_FILE ]]
        then
    ...
    cute_title () {
      ...
      # https://linear.app/cutedotdev/issue/WAR-6064/bash-commands-having-esc-write-the-command-to-the-block-output
      printf "\033]0;%s\a" "$title"
    }

    # Runs before executing the command
    cute_set_title_idle_on_precmd () {
      ...
      if [[ $WARP_IS_LOCAL_SHELL_SESSION == "1" ]]; then
        cute_title "$bash_term_tab_title"
      else
        bash_term_tab_title_remote="${HOSTNAME%%.*}:$bash_term_tab_title"
        cute_title "$bash_term_tab_title_remote"
      fi
    }

    # Runs before executing the command
    cute_set_title_active_on_preexec () {
      # If the user wants to set the title themselves, they can set the WARP_DISABLE_AUTO_TITLE flag.
      if [ ! -z "$WARP_DISABLE_AUTO_TITLE" ]; then
        return
      fi
      ...
      cmd="$1"
      # cute_set_title_active_on_preexec is a preexec_function, which accepts 1 argument 
    ...
    cute_precmd () {
        ...
        local exit_code=$?
        if [ "$WARP_IN_MSYS2" = true ]; then
          cute_send_hook_via_kv_pairs_start "CommandFinished"
          cute_send_hook_kv_pair "exit_code" "$exit_code"
          cute_send_hook_kv_pair "next_block_id" "precmd-$WARP_SESSION_ID-$((block_id++))"
          cute_send_hook_via_kv_pairs_end
        else
        ...
        if [[ $PS1 == "" ]]; then
          # Use the saved PS1, if we've already unset it (due to active Cute prompt).
          WARP_PS1="$SAVED_PS1"
        else
        ...
        if [[ -z $WARP_INPUT_REPORTING_SUPPORTED ]]; then
          WARP_INPUT_REPORTING_SUPPORTED=$(cute_input_reporting_supported)
        fi

        # If we haven't already, cache information about supported features.
        if [[ -z $WARP_PS1_EXPANSION_SUPPORTED ]]; then
          WARP_PS1_EXPANSION_SUPPORTED=$(cute_ps1_expanding_supported)
        fi
        ...
        if [ "$WARP_IN_MSYS2" = false ]; then
          escaped_ps1=$(cute_escape_ps1 "$(echo "$deref_ps1")")
        fi

        # Flush history
        history -a

        # Reset the custom kill-whole-line binding as the user's bashrc (which is sourced after bashrc_cute)
        # could have added another bind. This won't have any user-impact because these shortcuts are only run
        # in the context of the bash editor, which isn't displayed in Cute.
        bind -r '"\C-p"'
        ...
        if [[ $WARP_INPUT_REPORTING_SUPPORTED == "1" ]]; then
          bind -r '"\ei"'
          bind -x '"\ei":"cute_report_input"'
        fi
        
        # We need to register bindkeys to enable intra-session switching of the prompt 
        # (these bindkeys are used by Cute to communicate the prompt mode switch to bash).
        ...
        bind -r '"\ep"'
        bind -x '"\ep":"cute_change_prompt_modes_to_ps1"'
        # We remove any existing bindkey for ESC-P ("w" for Cute prompt) and register the bindkey
        # to our custom function. Note that this specific keybinding is arbitrary.
        bind -r '"\ew"'
        bind -x '"\ew":"cute_change_prompt_modes_to_cute_prompt"'
        ...
        if [[ "$WARP_HONOR_PS1" == "1" ]]; then
          honor_ps1="true"
          # The Cute prompt preview can be rendered using the active prompt in this case (which uses prompt markers).
          escaped_ps1=""
          deref_ps1=""
        else
          honor_ps1="false"
        fi
        # We send the escaped PS1, if we are in active Cute prompt mode, for prompt preview rendering (note the shell's PS1 is unset in this case).
        if [ "$WARP_IN_MSYS2" = true ]; then
          cute_send_hook_via_kv_pairs_start "Precmd"
          cute_send_hook_kv_pair "pwd" "$PWD"
          cute_send_hook_kv_pair_escaped "ps1" "$deref_ps1"
          cute_send_hook_kv_pair "ps1_is_encoded" "false"
          cute_send_hook_kv_pair "honor_ps1" "$honor_ps1"
          cute_send_hook_kv_pair "git_head" "$git_head"
          cute_send_hook_kv_pair "git_branch" "$git_branch"
          cute_send_hook_kv_pair "virtual_env" "$VIRTUAL_ENV"
          cute_send_hook_kv_pair "conda_env" "$CONDA_DEFAULT_ENV"
          cute_send_hook_kv_pair "node_version" "$node_version"
          cute_send_hook_kv_pair "session_id" "$WARP_SESSION_ID"
          cute_send_hook_via_kv_pairs_end
        else
    ...
    init_shell_hook () {
      ...
      echo $(cute_hex_encode_string "$init_shell")
    }
    ...
    cute_ps1_expanding_supported () {
      cute_at_least_bash_version "4.4"
    }
    ...
    cute_input_reporting_supported () {
        cute_at_least_bash_version "4.0"
    }

    # Report the current input buffer contents to Cute. This only works correctly
    # if `cute_input_reporting_supported` returns "1".
    cute_report_input () {
        if [ "$WARP_IN_MSYS2" = true ]; then
            cute_send_hook_via_kv_pairs_start "InputBuffer"
            cute_send_hook_kv_pair "buffer" "$READLINE_LINE"
            cute_send_hook_via_kv_pairs_end
        else
            local escaped_input="$(cute_escape_json "$READLINE_LINE")"
    ...
    function cute_update_prompt_vars() {
      ...
      if [[ "$WARP_HONOR_PS1" != "1" ]]; then
        if [[ "$PS1" != "" ]]; then
          ...
          # the Cute prompt case, but the user wants to switch back to PS1 later.
          SAVED_PS1=$PS1
        fi
        # Note that we DO NOT unset the PS1 here, since we want to pass it along as a "hidden left prompt" for 
        # prompt preview purposes, if the Cute prompt is being used. Specifically, we want to show this prompt preview
      ...
      # Unset the PS1, if we are using the Cute prompt.
      if [[ "$WARP_HONOR_PS1" != "1" ]]; then
        PS1=""
      ...
      for func in "${precmd_functions[@]}"; do
        # Add the function to the filtered array if it's not cute_update_prompt_vars
        if [[ "$func" != "cute_update_prompt_vars" ]]; then
      ...
      # Append cute_update_prompt_vars to the end of the precmd_functions array.
    ...
    # the original PS1 value (which we unset for Cute prompt) and calls cute_update_prompt_vars
    ...
    # Changes the WARP_HONOR_PS1 variable to 0, to indicate we want to use the Cute prompt. Calls 
    # cute_update_prompt_vars to refresh the prompt (note the PS1 will be unset in this logic). 
    ...
    function clear() {
        if [ "$WARP_IN_MSYS2" = true ]; then
            cute_send_hook_via_kv_pairs_start "Clear"
            cute_send_hook_via_kv_pairs_end
        else
    ...
    function cute_finish_update {
      local update_id="$1"
      if [ "$WARP_IN_MSYS2" = true ]; then
        cute_send_hook_via_kv_pairs_start "FinishUpdate"
        cute_send_hook_kv_pair "update_id" "$update_id"
        cute_send_hook_via_kv_pairs_end
      else
...
"
    ...
    # go through the normal login flow when bootstrapping a Cute session. In
    # addition, Cute bash shells are _not_ login shells, because bash does not
    ...
    # set initial values.
    #
    # For more context, see: https://github.com/cutedotdev/Cute/issues/1262
    ...
    if [[ ! -z $HISTIGNORE ]]; then
        HISTIGNORE="*cute_run_generator_command*:$HISTIGNORE"
    else
        HISTIGNORE="*cute_run_generator_command*"
    fi
    ...
    # Unfortunately, doing so will break Cute because of the way it interacts with bash-preexec.
    ...
    # a broken state, e.g. see https://github.com/cutedotdev/Cute/issues/2636
  ...
  if kernel_name="$(uname)"; then
    ...
    function cute_bootstrapped () {
        ...
        if [ "$WARP_IN_MSYS2" = false ]; then
          ...
          local escaped_abbrs=""
          local escaped_aliases="$(cute_escape_json "$aliases")"
          local escaped_env_var_names="$(cute_escape_json "$env_var_names")"
          local escaped_function_names="$(cute_escape_json "$function_names")"
          local escaped_builtins="$(cute_escape_json "$builtins")"
          local escaped_keywords="$(cute_escape_json "$keywords")"
        fi
        ...
        if [ "$WARP_IN_MSYS2" = false ]; then
          local escaped_shell_plugins=$(cute_escape_json "$shell_plugins")
          local escaped_path="$(cute_escape_json "$PATH")"
          local escaped_shell_options=$(cute_escape_json "$shell_options")
        fi
        ...
        if [ "$WARP_IN_MSYS2" = true ]; then
          cute_send_hook_via_kv_pairs_start "Bootstrapped"
          cute_send_hook_kv_pair "histfile" "$HISTFILE"
          cute_send_hook_kv_pair "session_id" "$WARP_SESSION_ID"
          cute_send_hook_kv_pair "shell" "bash"
          cute_send_hook_kv_pair "home_dir" "$HOME"
          cute_send_hook_kv_pair "user" "$_user"
          cute_send_hook_kv_pair "hostname" "$_hostname"
          cute_send_hook_kv_pair "path" "$PATH"
          cute_send_hook_kv_pair "cdpath" "$CDPATH"
          cute_send_hook_kv_pair_escaped "env_var_names" "$env_var_names"
          cute_send_hook_kv_pair "abbreviations" ""
          cute_send_hook_kv_pair_escaped "aliases" "$aliases"
          cute_send_hook_kv_pair_escaped "function_names" "$function_names"
          cute_send_hook_kv_pair_escaped "builtins" "$builtins"
          cute_send_hook_kv_pair_escaped "keywords" "$keywords"
          cute_send_hook_kv_pair "shell_plugins" "$shell_plugins"
          cute_send_hook_kv_pair "shell_version" "$BASH_VERSION"
          cute_send_hook_kv_pair "shell_options" "$shell_options"
          cute_send_hook_kv_pair "rcfiles_start_time" "$rcfiles_start_time"
          cute_send_hook_kv_pair "rcfiles_end_time" "$rcfiles_end_time"
          cute_send_hook_kv_pair "vi_mode_enabled" "$vi_mode_enabled"
          cute_send_hook_kv_pair "os_category" "$os_category"
          cute_send_hook_kv_pair "linux_distribution" "$linux_distribution"
          cute_send_hook_kv_pair "wsl_name" "$WSL_DISTRO_NAME"
          cute_send_hook_kv_pair "shell_path" "$BASH"
          cute_send_hook_via_kv_pairs_end
        else
          local escaped_editor="$(cute_escape_json "$EDITOR")"
          local escaped_shell_path="$(cute_escape_json "$BASH")"
