...
_err() {
    _log RemoteCuteificationIsUnavailable "$1"
}
...
_check_tmux() {
    if _find $HOME/.cute/tmux/execute_tmux.sh; then
        _log SshTmuxInstaller "\"cute\""
        TMUX="$HOME/.cute/tmux/execute_tmux.sh"
...
_check_tmux && command $TMUX -Lcute -CC && exit
