...
function _err
    _log RemoteCuteificationIsUnavailable $argv[1]
end
...
function _check_tmux
    ...
    else if _is $HOME/.cute/tmux/execute_tmux.sh
        set TMUX "$HOME/.cute/tmux/execute_tmux.sh"
        _log SshTmuxInstaller "\"cute\""
    end
...
_check_tmux; and command $TMUX -Lcute -CC; and exit
