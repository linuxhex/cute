...
- **Runtime** (`cute_core/src/features.rs`): `DOGFOOD_FLAGS`, `PREVIEW_FLAGS`, `RELEASE_FLAGS` — enabled per-channel at startup.
...
Add the flag to `DOGFOOD_FLAGS` in `cute_core/src/features.rs`:
...
1. Add to `PREVIEW_FLAGS` in `cute_core/src/features.rs`.
...
Prefer this over adding to `RELEASE_FLAGS` (see comment at `cute_core/src/features.rs:787-790`). It compiles the feature into all builds and enables a one-line rollback.
...
### 3. `cute_core/src/features.rs` — remove from `PREVIEW_FLAGS` / `DOGFOOD_FLAGS`
