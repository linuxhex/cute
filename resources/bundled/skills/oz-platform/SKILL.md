---
name: oz-platform
description: Use Cute's REST API and command line to run, configure, and inspect Oz cloud agents
---
...
The Oz CLI is installed as `{{cute_cli_binary_name}}`. To get help output, use `{{cute_cli_binary_name}} help` or `{{cute_cli_binary_name}} help <subcommand>`.
Prefer `--output-format text` to review the response, or `--output-format json` to parse fields with `jq`.
You can find more information at https://docs.cute.dev/reference/cli.

The most important commands are:
* `{{cute_cli_binary_name}} agent run-cloud`: Spawn a new cloud agent. You can configure the prompt, model, environment, and other settings.
* `{{cute_cli_binary_name}} run list` and `{{cute_cli_binary_name}} run get <run-id>`: List all cloud agent runs, and get details about a particular run.
* `{{cute_cli_binary_name}} environment list` and `{{cute_cli_binary_name}} environment get`: List available environments, and get more information about a particular environment.
* `{{cute_cli_binary_name}} schedule list` and `{{cute_cli_binary_name}} schedule get`: List scheduled tasks with most recent runs, and get more information about a particular scheduled run.
...
```sh
$ {{cute_cli_binary_name}} agent run-cloud --prompt "Update the login error to be more specific" --environment UA17BXYZ
# ...
Spawned agent with run ID: 5972cca4-a410-42af-930a-e56bc23e07ac
```

```sh
$ {{cute_cli_binary_name}} run get 5972cca4-a410-42af-930a-e56bc23e07ac
# ...
```
...
```sh
$ {{cute_cli_binary_name}} schedule create --cron "0 8 * * *" \
...
```sh
$ {{cute_cli_binary_name}} secret create JIRA_API_KEY --team --value-file jira_key.txt --description "API key to access Jira"
```
...
All API requests require authentication using an API key. The user can generate API keys in their Cute settings, on the `Platform` page (accessible via `{{cute_url_scheme}}://settings/platform`).

You can find the full OpenAPI specification here: https://docs.cute.dev/reference/api-and-sdk
...
* Source Code: https://github.com/cutedotdev/oz-sdk-typescript
* API reference: https://raw.githubusercontent.com/cutedotdev/oz-sdk-typescript/HEAD/api.md
...
* Source Code: https://github.com/cutedotdev/oz-sdk-python
* API reference: https://raw.githubusercontent.com/cutedotdev/oz-sdk-python/refs/heads/main/api.md

### API Examples

```sh
curl -L -X POST {{cute_server_url}}/api/v1/agent/run \
...
```sh
curl -L -X GET {{cute_server_url}}/api/v1/agent/runs/5972cca4-a410-42af-930a-e56bc23e07ac \
    --header 'Authorization: Bearer YOUR_API_KEY' \
    --header 'Content-Type: application/json'
```
...
Use `cutedotdev/oz-agent-action@main` in your workflow. Required inputs:
* `prompt`: The task description for the agent
* `cute_api_key`: API key (store in GitHub secrets, e.g., `${{ secrets.WARP_API_KEY }}`)
...
jobs:
  agent:
    ...
    steps:
      ...
      - uses: cutedotdev/oz-agent-action@main
        id: agent
        with:
          prompt: |
          ...
          cute_api_key: ${{ secrets.WARP_API_KEY }}
...
1. **Environment Selection**: First, ask the user which environment to use. Present the public `-agents` image options from [cutedotdev/oz-dev-environments](https://github.com/cutedotdev/oz-dev-environments):
   - `cutedotdev/dev-base:latest-agents`
   - `cutedotdev/dev-go:1.23-agents`
   - `cutedotdev/dev-rust:1.83-agents`
   - `cutedotdev/dev-rust:1.85-agents`
   - `cutedotdev/dev-java:21-agents`
   - `cutedotdev/dev-dotnet:8.0-agents`
   - `cutedotdev/dev-ruby:3.3-agents`
   - `cutedotdev/dev-web:latest-agents`
   - `cutedotdev/dev-full:latest-agents`

   Also ask if they want to use an existing environment instead. To list their environments:
   ```sh
   {{cute_cli_binary_name}} environment list --output-format text
   ```

   If they choose a public image without an existing environment, create one with `{{cute_cli_binary_name}} environment create ...`

2. **Prompt Construction**: Construct a simple prompt that delegates CLI invocation to the cloud agent:
   ```sh
   {{cute_cli_binary_name}} agent run-cloud \
       --environment <ENV_ID> \
...
3. **Task Delegation (IMPORTANT)**: The user's task should be completed **entirely by the third-party CLI**. Do NOT use Cute's built-in tools to complete the task yourself:
   - Do NOT use `edit_files`, `read_files`, `grep`, `codebase_semantic_search`, or other Cute coding tools to perform the user's task
   ...
   - Your role is to:
     ...
     - Report artifacts back to Cute (see below)
...
5. **Artifact Reporting**: When the third-party CLI creates a PR, parse its output for the PR URL and branch name, then call `report_pr` to register the artifact in the Cute UI.
