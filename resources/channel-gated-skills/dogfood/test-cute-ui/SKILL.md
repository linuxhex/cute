...
name: test-cute-ui
description: >
  Guides testing Cute UI features and changes using the computer use tool.
  Use this skill only when the computer_use tool is available to the agent.
  Covers launching Cute and verifying UI behavior.
user-invocable: false
---

# Computer Use for Cute UI Testing

Use the `computer_use` tool to visually test that Cute looks and behaves as intended after UI changes.

## Running Cute

Launch Cute from the repository root with:
...
- The command to build and launch Cute (typically `cargo run -- --api-key $STAGING_USER_WARP_API_KEY` from the repo root)
