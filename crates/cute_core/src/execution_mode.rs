...
/// Execution mode that Cute is running under.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ExecutionMode {
    /// Cute is running as a normal desktop app.
    App,
    /// Cute is running as a CLI.
    Sdk,
    /// Cute is running as the remote server daemon.
    RemoteServerDaemon,
}
...
impl ExecutionMode {
    ...
    pub fn client_id(&self) -> &'static str {
        match self {
            ExecutionMode::App => "cute-app",
            ExecutionMode::Sdk => "cute-cli",
            ExecutionMode::RemoteServerDaemon => "cute-remote-server-daemon",
        }
    }
}

/// Model tracking the mode that Cute is running in.
///
/// This gates functionality that's disabled when Cute is running in SDK mode.
#[derive(Clone, Debug)]
...
impl AppExecutionMode {
    ...
    /// If true, Cute is running in a sandbox like a Docker container or VM, rather than directly
    /// on a user machine.
...
/// Returns the current global client ID string ("cute-app", "cute-cli", or "cute-remote-server-daemon").
