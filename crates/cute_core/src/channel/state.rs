...
use crate::channel::config::{
    ChannelConfig, McpOAuthProviderConfig, OzConfig, RudderStackDestination, CuteServerConfig,
};
...
impl ChannelState {
    pub fn init() -> Self {
        ...
        let app_id = AppId::new("dev", "cute", "CuteOss");
        Self {
            channel,
            ...
            config: ChannelConfig {
                app_id,
                logfile_name: "".into(),
                server_config: CuteServerConfig::production(),
    ...
    pub fn uses_staging_server() -> bool {
        ...
        url.host_str() == Some("staging.cute.dev")
    }
    ...
    ///
    /// This is a debugging tool for isolating development instances of Cute, and is not
    ...
    /// telemetry events. Builds like OpenCute intentionally ship with
    ...
    /// ship crash reports. Builds like OpenCute intentionally ship with
    ...
    /// served by cute-server-rtc (e.g. the agent event SSE stream).
    ///
    ...
    pub fn url_scheme() -> &'static str {
        match Self::channel() {
            Channel::Stable => "cute",
            Channel::Preview => "cutepreview",
            Channel::Dev => "cutedev",
            // Dummy value--integration tests shouldn't support URL schemes.
            Channel::Integration => "cuteintegration",
            Channel::Local => "cutelocal",
            Channel::Oss => "cuteoss",
        }
    }
}
...
fn app_id_from_bundle() -> Option<AppId> {
    ...
    unsafe {
        ...
        use cuteui::platform::mac::utils::nsstring_as_str;
