...
/// in the Cute ecosystem. It enables events to be defined in any crate while maintaining
...
macro_rules! send_telemetry_from_ctx {
    ($event:expr, $ctx:expr) => {
        ...
        use cute_core::telemetry::TelemetryEvent as _;
        let event = $event;
        if event.enablement_state().is_enabled() {
            let auth_state =
                <$crate::telemetry::TelemetryContextModel as cuteui::SingletonEntity>::handle($ctx)
                    .as_ref($ctx);
            let user_id = auth_state.user_id($ctx);
            let anonymous_id = auth_state.anonymous_id($ctx);
            cuteui::record_telemetry_from_ctx!(
                user_id,
                anonymous_id,
                event.name().into(),
                event.payload(),
...
macro_rules! send_telemetry_from_app_ctx {
    ($event:expr, $app_ctx:expr) => {
        ...
        if event.enablement_state().is_enabled() {
            let auth_state =
                <$crate::telemetry::TelemetryContextModel as cuteui::SingletonEntity>::handle(
                    $app_ctx,
                )
                .as_ref($app_ctx);
            let user_id = auth_state.user_id($app_ctx.as_ref());
            let anonymous_id = auth_state.anonymous_id($app_ctx.as_ref());
            cuteui::record_telemetry_on_executor!(
                user_id,
                anonymous_id,
                event.name().into(),
                event.payload(),
