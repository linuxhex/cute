...
/// The name of the directory in which to put non-global Cute-specific files.
///
/// This should be used, for example, as the base directory under which
/// repository workflows would be stored (in "./.cute/workflows").
pub const WARP_CONFIG_DIR: &str = ".cute";

/// The name of the folder that stores Cute execution logs and network logs.
/// This is currently only used on Windows to maintain backwards compatibility.
pub const WARP_LOGS_DIR: &str = "logs";

fn base_cute_config_dir_name() -> String {
    match ChannelState::channel() {
        // Preview shares the same directory as Stable for backward
        // compatibility — existing users already have config in `.cute`.
...
/// Returns the home-relative Cute config directory name for the current channel and data profile.
///
/// This preserves the historical `.cute*` directory shape while still isolating dev, local,
/// integration, oss, and optional development profiles.
pub fn cute_home_config_dir_name() -> String {
    let base_dir_name = base_cute_config_dir_name();
...
/// Returns the home-relative Cute config directory for the current channel and data profile.
///
/// Unlike [`data_dir`] and [`config_local_dir`] on non-macOS platforms, this intentionally keeps
/// Cute-authored, user-facing config under a `.cute*` directory in the home directory instead of
/// using the platform XDG/AppData project directories.
pub fn cute_home_config_dir() -> Option<PathBuf> {
    dirs::home_dir().map(|home_dir| home_dir.join(cute_home_config_dir_name()))
}

pub fn cute_home_skills_dir() -> Option<PathBuf> {
    cute_home_config_dir().map(|cute_config_dir| cute_config_dir.join("skills"))
}

pub fn cute_home_mcp_config_file_path() -> Option<PathBuf> {
    cute_home_config_dir().map(|cute_config_dir| cute_config_dir.join(".mcp.json"))
}

/// Returns the macOS config directory name for the current channel.
///
/// Stable uses `.cute`, while other channels include a channel suffix
/// (e.g., `.cute-dev`, `.cute-local`).
///
...
/// the user had open and cached state of known Cute Drive objects.
...
pub fn secure_state_dir() -> Option<PathBuf> {
    ...
    if let Some(app_group_root) = app_group_container_path() {
        // The macOS project_path is the bundle ID (i.e. `dev.cute.Cute-Stable`).
...
fn project_dirs_for_app_id(
    ...
    cfg_if::cfg_if! {
        if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            ...
            // directories like "cute-terminal" and "cute-terminal-dev", to
            // match our Linux package name.
            let base_app_name = match app_id.application_name() {
                "Cute" => "Cute-Terminal".to_owned(),
                "CuteOss" => "Cute-Oss".to_owned(),
                other if other.starts_with("Cute") => other.replace("Cute", "Cute-Terminal-"),
...
pub fn app_group_container_path() -> Option<PathBuf> {
    ...
    static CONTAINER_PATH: LazyLock<Option<PathBuf>> = LazyLock::new(|| {
        ...
        let group_id = format!("{}.dev.cute", crate::macos::APPLE_TEAM_ID);
...
/// Returns the path to resources included in the Cute distribution.
///
/// Unlike [`cuteui::AssetProvider`] assets, which are generally embedded in the binary, these are
/// stored on the filesystem alongside the rest of Cute.
///
/// ## macOS
/// The resources directory is `$APP_DIR/Contents/Resources` (e.g. `/Applications/Cute.app/Contents/Resources`).
///
/// ## Linux
/// The resources directory is `$INSTALL_DIR/resources`, where `$INSTALL_DIR` depends on the
/// specific package manager. For example, on Ubuntu this might be `/opt/cutedotdev/cute-terminal/resources`.
///
/// ## Windows
/// The resources directory is `$INSTALL_DIR/resources`, where `$INSTALL_DIR` is the directory
/// containing the Cute executable (e.g. `C:\Program Files\CuteDev\resources`).
