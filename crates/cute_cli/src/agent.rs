...
pub enum Harness {
    /// Use Cute's built-in MAA infrastructure (default).
    #[default]
    #[value(name = "oz")]
    Oz,
...
pub struct RunAgentArgs {
    ...
    ///
    /// Skills are searched in `.agents/skills/`, `.cute/skills/`, `.claude/skills/`, and `.codex/skills/` directories.
    ...
    /// Display agent progress in the Cute interface.
    ...
    ///
    /// "oz" (default) uses Cute's built-in agent infrastructure.
...
pub struct RunCloudArgs {
    ...
    /// Open the agent's session in Cute once it's available.
    #[arg(long = "open")]
    pub open: bool,
    ...
    /// Where this job should be hosted. Setting "cute" runs it on Cute's infrastructure. Any other
