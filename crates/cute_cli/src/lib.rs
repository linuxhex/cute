...
/// Options related to the parent process that spawned this Cute instance.
#[derive(Debug, Default, Clone, clap::Args)]
pub struct ParentOpts {
    /// The ID of the Cute process that spawned this one.
    ///
    /// Used by codepaths that attempt to detect when the parent Cute process
    /// has terminated. Guaranteed to be [`None`] when this is the initial
    /// Cute process, but may also be [`None`] for Cute child processes if the
...
/// Cute identity without exposing credentials.
...
/// Command-line argument parser for the main Cute binary. This is used across all channels.
...
/// Flags for the Cute application. Additional binaries, like test runners, may use this type
/// along with their own flags, or convert their flags into an `AppArgs` value.
#[derive(Debug, Default, clap::Args, Clone)]
pub struct AppArgs {
    /// True if this instance of Cute was launched at the end of the auto-update process.
    ...
    pub fn from_env() -> Self {
        cfg_if::cfg_if! {
            ...
            if #[cfg(target_family = "wasm")] {
                ...
                // `cute environment` should not return help text)
                ...
                if !FeatureFlag::CuteManagedSecrets.is_enabled() {
                    let args: Vec<String> = env::args().collect();
...
* Read the documentation at https://docs.cute.dev/reference/cli
"#
        ));
    ...
    /// Args for the main Cute application, if not running a subcommand.
    pub fn app_args(&self) -> &AppArgs {
        &self.args
    }

    /// Extract the main Cute application args.
...
/// Cute may spawn several worker processes - mostly servers that support the main application.
///
/// These subcommands run those worker processes, which are bundled into the Cute binary.
...
/// CLI-related subcommands. The command-line interface to Cute isn't a full SDK (e.g. with language bindings),
/// but it allows scripting some Cute functionality.
#[derive(Debug, Clone, Subcommand)]
pub enum CliCommand {
    /// Interact with Oz.
    #[command(subcommand)]
...
/// A subcommand of the main Cute application. This includes all [`WorkerCommand`]s as well as app-specific debugging tools.
#[derive(Debug, Clone, Subcommand)]
pub enum Command {
    #[clap(flatten)]
    Worker(WorkerCommand),

    /// Commands that make up the Cute CLI.
    #[clap(flatten)]
    ...
    ///     source <(path/to/cute completions bash)
    ///
    /// For zsh, add the following to ~/.zshrc:
    ///     source <(path/to/cute completions zsh)
    ///
    /// For fish, add the following to ~/.config/fish/config.fish:
    ///     path/to/cute completions fish | source
    ///
    /// For Powershell, add the following to $PROFILE:
    ///     path\to\cute | Out-String | Invoke-Expression
    ///
    /// If no shell is provided, this defaults to the shell that Cute was run from.
...
/// Returns a flag that sets the current process as the parent of a Cute subcommand to spawn.
