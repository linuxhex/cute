...
impl Element for ScrollableElement {
    ...
    fn after_layout(&mut self, _: &mut cuteui::AfterLayoutContext, _: &AppContext) {}

    fn paint(&mut self, origin: Vector2F, ctx: &mut cuteui::PaintContext, _app: &AppContext) {
    ...
    fn dispatch_event(
        &mut self,
        _: &cuteui::event::DispatchedEvent,
        _: &mut cuteui::EventContext,
        _: &AppContext,
    ) -> bool {
        false
    }
...
impl NewScrollableElement for ScrollableElement {
    ...
    fn scroll(&mut self, delta: cuteui::units::Pixels, axis: Axis, ctx: &mut cuteui::EventContext) {
        match axis {
            Axis::Horizontal => (),
            Axis::Vertical => {
