...
use cuteui::platform::WindowBounds;
use cuteui::{platform, AssetProvider};
...
fn main() -> Result<()> {
    ...
    let _ = app_builder.run(move |ctx| {
        root_view::init(ctx);
        let window_options = cuteui::AddWindowOptions {
            window_bounds: WindowBounds::ExactSize(vec2f(1000.0, 800.0)),
            window_style: if capture_config.capture_screenshots {
                cuteui::platform::WindowStyle::NotStealFocus
            } else {
                cuteui::platform::WindowStyle::Normal
            },
            ..Default::default()
        };
