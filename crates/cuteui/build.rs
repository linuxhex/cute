...
fn main() {
    ...
    if env::var("CARGO_CFG_TARGET_OS").as_deref() == Ok("macos") {
        ...
        // Skip Metal shader compilation if SKIP_METAL_SHADERS is set
        if env::var("SKIP_METAL_SHADERS").is_err() {
            compile_metal_shaders();
        } else {
            // Create an empty metallib file so that include_bytes! doesn't fail
            let out_path = PathBuf::from(env::var("OUT_DIR").unwrap());
            let lib_path = out_path.join("shaders.metallib");
            std::fs::write(&lib_path, []).expect("Couldn't write empty metallib file");
            println!("cargo:rustc-env=SKIP_METAL_SHADERS=1");
        }
        compile_objc_lib();
    }
}
