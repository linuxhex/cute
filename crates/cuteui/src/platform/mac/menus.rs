...
use cuteui_core::keymap::Keystroke;
use cuteui_core::platform::menu::{
...
#[no_mangle]
extern "C-unwind" fn cute_menu_item_needs_update(item: id, ctx: *mut c_void) {
...
#[no_mangle]
extern "C-unwind" fn cute_menu_item_triggered(_item: id, ctx: *mut c_void) {
    let func = &MenuItemData::read_context(ctx).triggered;
    callback_dispatcher().menu_item_triggered(func);
}

#[no_mangle]
extern "C-unwind" fn cute_menu_item_deallocated(ctx: *mut c_void) {
...
fn resolve_standard_action(action: StandardAction) -> StandardMenuItemProperties {
    ...
    match action {
        ...
        StandardAction::Quit => make("Quit Cute", "terminate:", cmd, "q"),
        StandardAction::Hide => make("Hide Cute", "hide:", cmd, "h"),
...
unsafe fn apply_changes(changes: MenuItemPropertyChanges, item: id) {
    // Wrap in a local autorelease pool: AppKit invokes `cute_menu_item_needs_update`
