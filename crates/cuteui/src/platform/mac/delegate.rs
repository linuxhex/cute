...
use cuteui_core::{platform, ApplicationBundleInfo, WindowId};
...
impl platform::Delegate for AppDelegate {
    ...
    fn open_character_palette(&self) {
        ...
        dispatch::Queue::main().exec_async(move || unsafe {
            ...
            // first responder's cursor location. In our case, that will be the Cute
...
pub unsafe extern "C-unwind" fn cute_on_request_notification_permissions_completed(
...
/// This function is marked unsafe because it retrieves the pointer to the callback
/// function that we sent down to the Objective-C code.
pub unsafe extern "C-unwind" fn cute_on_notification_send_error(
