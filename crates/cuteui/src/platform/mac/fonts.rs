...
use cuteui_core::fonts::canvas::RasterFormat;
use cuteui_core::fonts::{
    FamilyId, FontId, FontInfo, GlyphId, Metrics, Properties, RasterizedGlyph, SubpixelAlignment,
};
use cuteui_core::platform::{self, FontDB as _, LineStyle, TextLayoutSystem};
use cuteui_core::rendering;
use cuteui_core::text_layout::{ClipConfig, StyleAndFont, TextAlignment, TextFrame};
...
mod loader {
    ...
    pub fn load_system_font(font_family: &str) -> Result<FontFamily> {
        ...
        for fontdesc in descriptors.into_iter() {
            ...
            let font = unsafe { Font::from_native_font(font::new_from_descriptor(&fontdesc, DEFAULT_FONT_SIZE)) };
    ...
    // We use font-kit's family handle to load fonts that come with Cute as
...
impl FontDB {
    ...
    fn descriptor_to_font_id(&self, fontdesc: ItemRef<CTFontDescriptor>) -> Option<FontId> {
        ...
        (!name.starts_with('.')).then(|| {
            ...
            self.push_font(unsafe { Font::from_native_font(font::new_from_descriptor(
                &fontdesc,
                DEFAULT_FONT_SIZE,
            )) })
        })
    ...
    pub fn select_font(&self, family_id: FamilyId, properties: Properties) -> FontId {
        match self.font_selections.entry((family_id, properties)) {
            ...
            Entry::Vacant(entry) => {
                ...
                let font_id = {
                    // Simple font matching: find the first font that matches weight/style
                    // or fall back to the first font in the family
                    let query_props = properties_to_font_kit(properties);
                    let mut best_idx = 0;
                    let mut best_score = i32::MAX;

                    for (idx, candidate) in candidates.iter().enumerate() {
                        let weight_diff = (candidate.weight.0 as i32 - query_props.weight.0 as i32).abs() as i32;
                        let style_match = if candidate.style == query_props.style { 0 } else { 100 };
                        let score = weight_diff + style_match;
                        if score < best_score {
                            best_score = score;
                            best_idx = idx;
                        }
