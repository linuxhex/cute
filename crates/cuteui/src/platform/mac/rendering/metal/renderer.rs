...
use cuteui_core::fonts::{self, canvas, RasterizedGlyph, SubpixelAlignment};
use cuteui_core::platform::CapturedFrame;
use cuteui_core::rendering::texture_cache::TextureCache;
use cuteui_core::rendering::{self};
use cuteui_core::scene::{CornerRadius, GlyphFade, GlyphKey, Icon, Image, Layer, Scene};
...
impl super::super::Renderer for Renderer {
    fn render(&mut self, scene: &Scene, window: &WindowState, font_cache: &fonts::Cache) {
        // SAFETY: `render` is called via `cute_update_layer`, which is only be invoked for
