...
//!
//! ## How does a11y work in Cute?
//! Because Cute uses its own rust UI framework (cuteui), we don’t benefit from the built-in
...
//!
//! CuteUI framework right now provides 3 ways of announcing what’s happening in the app:
...
pub struct AccessibilityContent {
    ...
    /// Frame support is a work-in-progress in Cute and right now this field is omitted and not set.
    pub frame: Option<RectF>,
    /// The role a given element has. Note that we use our own, CuteUI-defined roles (vs those that
...
impl AccessibilityContent {
    ...
    pub fn new_without_help<T>(value: T, role: CuteA11yRole) -> Self
    where
        T: Into<String>,
    {
    ...
    pub fn new<V, H>(value: V, help: H, role: CuteA11yRole) -> Self
    where
        V: Into<String>,
        H: Into<String>,
    {
    ...
    fn new_internal<V, H>(value: V, help: Option<H>, role: CuteA11yRole) -> Self
    where
        V: Into<String>,
        H: Into<String>,
    {
...
impl std::fmt::Display for CuteA11yRole {
...
impl ActionAccessibilityContent {
    pub fn from_debug() -> Self {
        Self::CustomFn(|action| {
            AccessibilityContent::new_without_help(format!("{action:?}."), CuteA11yRole::UserAction)
        })
    }
}
