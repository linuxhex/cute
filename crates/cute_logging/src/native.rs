...
/// 1) Renaming the log file from the main process (which just panicked) to `cute.log.old.temp`.
/// 2) Moving the crash recovery process log (which is located at `cute.log.recovery`) to the usual
///    path cute logs are located (log_directory/cute.log).
///    The temp log file (`cute.log.old.temp`) will ultimately be rotated to `cute.log.old.0` the next
...
pub async fn rotate_files(channel_file_name: &str, max_rotation: usize) -> Result<()> {
    ...
    // Rename `cute.log.old.temp` (the temporary file) to `cute.log.old.0`.
...
/// location at `log_directory/cute.log`.
...
/// Collects a list of the paths to both the current cute instance's log file,
...
fn sentry_log_filter(md: &log::Metadata) -> sentry_log::LogFilter {
    if cute_core::errors::should_ignore_log_for_sentry(md) {
    ...
    match md.target() {
        ...
        "cuteui::core::redraw_frame" => sentry_log::LogFilter::Ignore,
        ...
        t if t.starts_with("cute::crash_reporting::") => sentry_log::LogFilter::Ignore,
...
fn init_internal(
    ...
    /// Returns an empty file named `cute.log` to log the current execution, and
    ...
    fn setup_log_files_for_current_execution(
        ...
        let main_log_path = if is_from_crash_recovery_process {
            ...
            // not rename the old main log file to `cute.log.temp` like we do below because this
            ...
            // Rename the old main log file to `cute.log.temp`.
