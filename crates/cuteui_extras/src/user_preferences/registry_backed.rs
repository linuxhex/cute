...
static WARP_REGISTRY_BASE_PATH: &str = "Software\\Cute.dev\\";
...
impl RegistryBackedPreferences {
    ...
    /// Gets Cute's registry key, creating it if it does not already exist.
    fn get_cute_registry(&self) -> Result<Key, super::Error> {
        CURRENT_USER.create(self.app_key_path.clone()).map_err(|e| {
            log::error!("unable to access Cute app key in Windows Registry: {e:#}");
