...
use crate::{FromCuteJs, IntoCuteJs, JsFunctionId, SerializedJsValue, TypedJsFunctionRef};

impl<I, O> TypedJsFunctionRef<I, O>
where
    I: for<'a> IntoCuteJs<'a> + DeserializeOwned + Clone + 'static,
    O: for<'a> FromCuteJs<'a> + Serialize + Clone + 'static,
{
...
impl JsFunctionRegistry {
    ...
    ///
    /// `I` and `O` must have corresponding `IntoCuteJs`/`FromCuteJs` implementations so they
...
pub trait CallableJsFunction {
    ...
    /// `IntoCuteJs`-implemmenting Rust type, and returns the serialized bytes representation of
    /// the function's output (which is of a Rust type that implemenets `FromCuteJs`).
    fn call(
        &self,
    ...
    O: for<'a> FromCuteJs<'a> + Serialize + Clone + 'static,
{
    fn call(
        ...
        let input_value = input.into_cute_js(ctx)?;
        let func = self.js_function.clone().restore(ctx)?;
        let output = O::from_cute_js(ctx, func.call((input_value,))?, function_registry)?;
...
///
/// `I` is the type of the function's input, which must implement `IntoCuteJs`.
/// `O` is the type of the function's return value, which must implement `FromCuteJs`.
#[derive(Clone)]
...
where
    I: for<'a> IntoCuteJs<'a> + DeserializeOwned + 'static,
    O: for<'a> FromCuteJs<'a> + Serialize + 'static,
{
