...
use super::{FromCuteJs, JsFunctionRegistry};
    ...
    T: FromCuteJs<'js>,
{
    ...
    if value.is_array() {
        Vec::<T>::from_cute_js(ctx, value, js_function_registry)
    } else {
        Ok(vec![T::from_cute_js(ctx, value, js_function_registry)?])
    }
}
    ...
    T: FromCuteJs<'js>,
{
    ...
    T::from_cute_js(ctx, value, js_function_registry)
}
