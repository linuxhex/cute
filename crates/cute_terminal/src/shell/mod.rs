...
// Stub types for CommandExitStatus and CommandOutput (previously from cute_completer)
...
#[cfg(windows)]
use cute_core::paths::base_config_dir;
...
impl Shell {
    ...
    ///
    /// These sequences are bound to Cute shell functions during session bootstrap that print the
...
impl From<ShellType> for cute_util::path::ShellFamily {
...
impl ShellType {
    ...
    /// If true, Cute will bootstrap the shell if it's the login shell on the remote host.
                        ...
                        output_string
                            ...
                            .flat_map(|line| {
                                ...
                                for ext in PATHEXT {
                                    ...
                                    // Cute about this command as-is and also sans-suffix, e.g.
                                    // "git" and "git.exe".
...
impl ShellLaunchData {
    ...
    fn to_native_path_encoding(&self, path_str: &str) -> Option<PathBuf> {
        match self {
            ...
            // The container is Unix; Cute runs on the host, so paths are
