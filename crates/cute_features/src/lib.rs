...
pub enum FeatureFlag {
    ...
    /// If `true`, fetch updated Cute channel versions from the Cute server endpoint instead of
    /// from GCP directly.
    FetchChannelVersionsFromCuteServer,
    ...
    /// Used to gate an experiment we're doing on CuteDev ONLY
    ...
    /// Whether the user is part of the Cute Alpha Program (AI Trusted Testers).
    ...
    /// Enable multiselect in Notebooks and Cute Text.
    RichTextMultiselect,
    ...
    /// Enables next action prediction within Cute, powered by AI.
    AgentPredict,

    /// Enables receiving shared Cute Drive objects.
    SharedWithMe,
    ...
    /// https://github.com/cutedotdev/session-sharing-server/blob/b6590ebd0b0e7f6847d6b2228b4e77d63939ce22/server/Cargo.toml#L13
    SessionSharingAcls,
    ...
    /// Playground for reducing Cute UI clutter.
    MinimalistUI,
    ...
    /// Adds aliases for executing Cute Drive workflows.
    WorkflowAliases,
    ...
    /// Enables support for Cute Packs.
    CutePacks,
    ...
    /// Enables Cute Drive objects (like workflows) as context in AI context menu
    DriveObjectsAsContext,

    /// Expands code diff edits to replace the current pane instead of opening in a new tab.
    ExpandEditToPane,
    /// Enables fallback model load output messaging in the cuteing indicator.
    ...
    /// Enables the /create-environment slash command for setting up Cute Environments
    ...
    /// Enables Cute Managed Secrets functionality.
    CuteManagedSecrets,
    ...
    /// Enables agent tips displayed below the cuteing indicator in Agent Mode.
    AgentTips,
    ...
    /// Enables auto-syncing ambient plans to Cute Drive.
    SyncAmbientPlans,

    /// Enables platform skills support (--skill flag) for agent runs.
    ///
    /// Skills are loaded from `.agents/skills/`, `.cute/skills/`, `.claude/skills/`, and `.codex/skills/`
    ...
    /// Enables the OpenCute launch modal announcing Cute going open-source.
    ...
    /// Ctrl-G intercepts the keystroke and opens Cute's input editor instead of $EDITOR.
    CLIAgentRichInput,
    ...
    /// Enables the new settings to control visibility of Cute Drive, Code Review Panel,
    /// and Project Explorer & Global Search features.
    OpenCuteNewSettingsModes,
    ...
    /// CLIs (e.g. `claude`) to execute prompts instead of Cute's agent harness.
    AgentHarness,

    /// Enables workspace- and block-snapshot handoff between cloud agent runs
    /// and the local Cute client.
    /// When enabled:
    ...
    /// Enables the install/update chip for the OpenCode Cute plugin.
    /// Requires HOANotifications to also be enabled.
    OpenCodeNotifications,

    /// Enables the install/update chip for the Codex Cute notification plugin.
    /// Requires HOANotifications to also be enabled.
    CodexNotifications,

    /// Enables the install/update chip for the Gemini CLI Cute extension.
    /// Requires HOANotifications to also be enabled.
    GeminiNotifications,
    ...
    /// Replaces the in-block cuteification banner with a cuteify footer.
    CuteifyFooter,
...
pub const DOGFOOD_FLAGS: &[FeatureFlag] = &[
    ...
    // These are enabled via 100% experiment on prod cute-server,
...
/// Features enabled for feature preview build users (e.g.: Friends of Cute).
/// All PREVIEW_FLAGS are also automatically added to dogfood builds (CuteDev).
...
/// Features enabled for all release builds (i.e.: everything but CuteLocal).
...
impl FeatureFlag {
    ...
    pub fn flag_description(&self) -> Option<&'static str> {
        ...
        match self {
            ...
            CloudEnvironments => Some("Enables creating and managing Cute Environments via the CLI."),
            CreateEnvironmentSlashCommand => Some("Enables the /create environment slash command for setting up Cute Environments with custom configurations."),
            ...
            SettingsFile => Some("Enables configuring Cute via a user-editable `settings.toml` file, with hot reload and error reporting for invalid values."),
