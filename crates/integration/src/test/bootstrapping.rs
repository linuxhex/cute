...
use cute::integration_testing::tab::tab_title_step;
...
use cute::terminal::session_settings::HonorPS1;
use cute::terminal::shell::{self, ShellType};
use cute::workspace::Workspace;
use cuteui::clipboard::ClipboardContent;
use cuteui::integration::TestStep;
use cuteui::{async_assert, async_assert_eq, ViewHandle};
...
pub fn test_paste_and_type_characters_before_bootstrap() -> Builder {
    new_builder()
        ...
        .with_step(
            TestStep::new("Cute input should not start focused, since .rc file is reading user input")
        ...
        .with_step(
            TestStep::new("Pasted text go into the pty and not cute input")
        ...
        .with_step(
            TestStep::new("Typed characters should go to the pty and not cute input")
        ...
        .with_step(wait_until_bootstrapped_single_pane_for_tab(0))
        .with_step(
            new_step_with_default_assertions("Cute input should be focused and keep buffered text")
                .add_assertion(input_editor_is_focused(0))
                .add_assertion(input_contains_string(0, "this is the pasted textthese are some typed characters".to_owned()))
        )
}
...
/// https://github.com/cutedotdev/Cute/issues/2636
