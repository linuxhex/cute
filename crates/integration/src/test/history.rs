...
use cute::integration_testing::command_search::{
    assert_command_search_has_results, assert_command_search_is_open,
    assert_history_filter_is_active,
};
use cute::integration_testing::input::assert_workflow_info_box_is_open;
...
use cute::integration_testing::view_getters::single_input_view;
use cute::integration_testing::{self};
use cute::search::command_search::settings::ShowGlobalWorkflowsInUniversalSearch;
use cute::sqlite_testing::set_user_and_hostname_for_commands;
use cute::terminal::input::Input;
use cute::terminal::model::session::get_local_hostname;
use cute::terminal::shell::ShellType;
use cuteui::{async_assert, ViewHandle};
...
/// up in Cute -- so we effectively do a "left join" on commands from the histfile with commands
/// loaded from sqlite.
