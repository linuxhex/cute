...
### Build and Run
- `cargo run` - Build and run Cute locally
- `cargo bundle --bin cute` - Bundle the main app

### Running with local cute-server
To connect Cute client to a local cute-server instance:
...
- `./script/run-clang-format.py -r --extensions 'c,h,cpp,m' ./crates/cuteui/src/ ./app/src/` - Format C/C++/Obj-C code
...
- `crates/cuteui/` and `crates/cuteui_core/` - Custom UI framework
...
5. **Cloud Sync**: Objects can be synchronized across devices via Cute Drive

### Development Guidelines

**Workspace Structure**:
- This is a Cargo workspace with 60+ member crates
- Main binary is in `app/`, UI framework in `crates/cuteui/`
...
Cute uses compile-time feature flags with a small runtime plumbing layer.
